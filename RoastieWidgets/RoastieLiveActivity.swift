...
struct RoastieLiveActivity: Widget {
        ...
        ActivityConfiguration(for: RoastieActivityAttributes.self) { context in
            ...
            DynamicIsland {
                DynamicIslandExpandedRegion(.leading) {
                    miniRing(progress: context.state.sleepProgress, color: DrJayTheme.sleep, icon: "moon.zzz.fill")
                        .padding(.leading, 6)
                }
                DynamicIslandExpandedRegion(.trailing) {
                    miniRing(progress: context.state.waterProgress, color: DrJayTheme.water, icon: "drop.fill")
                        .padding(.trailing, 6)
                }
                ...
                Image(systemName: "moon.zzz.fill")
                    .foregroundStyle(context.state.sleepGoalMet == false ? DrJayTheme.ugly : DrJayTheme.sleep)
            } compactTrailing: {
                Text("\(context.state.waterBottlesLogged)/\(context.state.waterGoalBottles)")
                    .font(.caption2.weight(.semibold))
                    .foregroundStyle(DrJayTheme.water)
            } minimal: {
    ...
    private func statusChip(label: String, icon: String, wasRoast: Bool) -> some View {
        HStack(spacing: 4) {
            Image(systemName: icon).font(.caption2).foregroundStyle(wasRoast ? DrJayTheme.roast : DrJayTheme.primary)
...
private struct LockScreenView: View {
    ...
    var body: some View {
        HStack(spacing: 16) {
            VStack(spacing: 4) {
                ring(progress: state.sleepProgress, color: DrJayTheme.sleep, icon: "moon.zzz.fill")
                Text(state.sleepHours.map { String(format: "%.1fh", $0) } ?? "–")
                    .font(.caption2)
                    .foregroundStyle(.white.opacity(0.7))
            }
            VStack(spacing: 4) {
                ring(progress: state.waterProgress, color: DrJayTheme.water, icon: "drop.fill")
